...
>
> Son güncelleme: 2026-07-09. Kod değişince burayı da güncelle.
...
| Sistem | Durum |
|---|---|
| **Han / Tavern (Saloon)** | Ekran yok, düğüm tipi yok. Harita düğümleri: `savas`/`elit`/`boss`/`dukkan`/`olay`/`saman`/`revir`/`mezar` |
| **Söylentiler (Rumors)** | `data/rumors/` yok, `RumorData` scripti yok, kod yok |
| **Ün (Reputation)** | Hiçbir yerde kazanılmıyor/harcanmıyor |
| **Anı (Memory)** | Yok — meta para birimi **Kalıntı** olarak yazılmış |
| **Emir Kartları** | `data/orders/` yok. Tek iz: Kumandan yeteneği (`battle_screen._on_commander`) |
| Garnizon "Söylenti Defteri" / "Han Loncası" | Dokümanda var, kodda yok |
